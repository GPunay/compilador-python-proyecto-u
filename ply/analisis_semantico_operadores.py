...
def get_tipo(lexema):
    global tokens_list
    temp=tokens_list
    if isinstance(lexema, (int)):
         return 'INT'
    if isinstance(lexema, (float)):
        return 'FLOAT'          
    for token in temp:
        ...
        if (p[3]!='='):
            ...
            if tipo[3]=='VAR':#Se hacen evaluaciones si se esta inicializando con una variable ya existenete
                if  declarada(p[4]):#Si la variable si existe
                    tipo[4]=token2[3]
                    if token2[3]==tipo[0]:#Si las variables son del mismo tipo
                        cadena = [id[1], p[2], categoria[1], tipo[0], token2[4], linea]
                        p[0]=p[2]
                    elif tipo[3]=='FLOAT' and tipo[0]=='INT':
                        cadena = [id[1], int(p[2]), categoria[1], tipo[0], token2[4], linea]
                        p[0]=int(p[2])
                    else:
                        errores+="Error: No se puede inicializar una variable tipo " + tipo[0] + " como una dato tipo " + tipo[3] + "\n"
                        p_error(p)
                else:
            ...
            #bool  
                ...
                p[0]=p[2]
                    ...
                    cadena = [id[1], p[2], categoria[1], tipo[0], int(p[4]), linea]
                    p[0]=p[2]
            else:#Si el tipo de dato no es el adecuado
                errores+="Error de Constante: No se puede inicializar una variable tipo " + tipo[0] + " como una dato tipo " + tipo[3] + "\n"
                p_error(p)
            #expresiones
...
def p_factor_float(p):#Se aceptan numero floatantes
    'factor : FLOAT'
    p[0]=p[1]
    #Para detectar valores booleanos
def p_factor_bool(p):
    'factor : BOOL'
    p[0]=p[1]
...
def p_condicional(p):
    '''condicional : RESERV condiciones RLLAVE fin_con LLLAVE
                    | RESERV condiciones RLLAVE fin_con LLLAVE RESERV RLLAVE fin_con_else'''
    #print("if")
    #prueba(p)
    global errores
...
def p_fin_condi_else(p):
    '''fin_con_else : bloques LLLAVE'''
...
def p_while(p):
    '''ciclo : RESERV condiciones RLLAVE fin_con LLLAVE RESERV POINTCOMA'''
    #print("while")
    #prueba(p)
